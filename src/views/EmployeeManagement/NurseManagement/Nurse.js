...
import { getAllNurses, addNurse, updateNurse, deleteNurse } from './NurseAPI'
const dummyPhysios = [
  {
    id: 1,
...
const PhysioManagement = () => {
 ...
 setPhysios(dummyPhysios)
  ...
  const handleSave = async (data) => {
    if (selectedPhysio) {
      await updatePhysio(hospitalId, selectedPhysio.id, data)
    } else {
      await addPhysio({ ...data, hospitalId })
    }
    fetchPhysios()
  ...
  try {
    await deleteNurse(hospitalId, id)
    fetchPhysios()
  } catch (error) {
