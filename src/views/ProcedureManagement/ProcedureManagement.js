...
export default function TherapyManagement() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaveLoading(true)
      const payload = { clinicId, branchId, therapyName: form.therapyName, exerciseIds: form.exercisesIds }
      if (editId) {
  ...
  const selectStyles = {
    ...
    menu: (base) => ({ ...base, borderRadius: "7px", border: "0.5px solid #d0dce9", boxShadow: "0 4px 16px rgba(0,0,0,0.08)", zIndex: 99999 }),
  ...
  const getExerciseList = (therapy) => {
    ...
    if (therapy.exercises && typeof therapy.exercises === "object") return Object.values(therapy.exercises)
    return []
  }
    ...
    <>
      ...
      <div className="tm-page-header">
        <div className="tm-page-title-group">
          <div className="tm-page-icon"><Stethoscope size={20} /></div>
          <div>
            ...
            {filteredList.length === 0 ? (
              ...
              displayData.map((item, index) => (
                <CTableRow key={item.id} className="tm-tr">
                  <CTableDataCell className="tm-td tm-td-num">
                  ...
                  <CTableDataCell className="tm-td">
                    <div className="tm-actions">
                      {can("Therapy Management", "read") && (
                        <button className="tm-action-btn view" title="View" onClick={() => handleView(item.id)}>
                          <Eye size={14} />
                        </button>
                      )}
                      {can("Therapy Management", "update") && (
                        <button className="tm-action-btn edit" title="Edit" onClick={() => handleEdit(item)}>
                          <Edit2 size={14} />
                        </button>
                      )}
                      {can("Therapy Management", "delete") && (
                        <button
                          className="tm-action-btn del" title="Delete"
                          onClick={() => { setServiceIdToDelete(item.id); setIsModalVisible(true) }}
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
      ...
      {/* ── ADD / EDIT MODAL ── */}
      <CModal visible={modal} onClose={resetForm} backdrop="static" alignment="center" className="tm-custom-modal">
        <CModalHeader className="tm-modal-header">
          <CModalTitle className="tm-modal-title">{editId ? "Edit" : "Add"} Therapy</CModalTitle>
        </CModalHeader>
        ...
        <CModalBody className="tm-modal-body">
          <CForm>
            <CRow>
              <CCol md={12}>
                <div className="tm-field">
                  <CFormLabel className="tm-label">Therapy Name <span className="tm-req">*</span></CFormLabel>
                  <CFormInput
              ...
              <CCol md={12}>
                <div className="tm-field">
                  <CFormLabel className="tm-label">Select Exercises <span className="tm-req">*</span></CFormLabel>
                  <Select
                    ...
                    menuPlacement="top"
                    onChange={(val) =>
                      setForm({ ...form, exercises: val || [], exercisesIds: val ? val.map((v) => String(v.value)) : [] })
                    }
                  />
            ...
            <div className="tm-modal-footer">
              <button type="button" className="tm-btn-secondary" onClick={resetForm}>Cancel</button>
              <button type="button" className="tm-btn-primary" onClick={handleSave} disabled={saveLoading}>
                ...
                ) : editId ? "Update Therapy" : "Save Therapy"}
              </button>
            </div>
          </CForm>
        </CModalBody>
      ...
      <CModal
        visible={viewModal}
        onClose={() => { setViewModal(false); setViewTherapy(null); setViewError(null) }}
        size="xl" backdrop="static" alignment="center" className="tm-custom-modal"
      >
        <CModalHeader className="tm-modal-header">
        ...
        <CModalBody className="tm-modal-body tm-view-body">
          ...
          {!viewLoading && viewError && (
            <div className="tm-view-state tm-view-error">
              <Stethoscope size={36} />
              <p>{viewError}</p>
              <button className="tm-btn-secondary" onClick={() => setViewModal(false)}>Close</button>
            </div>
          )}
          ...
          {!viewLoading && !viewError && viewTherapy && (
            <>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "16px" }}>
                <button className="tm-btn-secondary" onClick={() => { setViewModal(false); setViewTherapy(null); setViewError(null) }}>
                  Close
                </button>
              </div>
            </>
      ...
      <ConfirmationModal
        ...
        confirmText={delloading ? (<><span className="spinner-border spinner-border-sm me-2 text-white" />Deleting...</>) : "Yes, Delete"}
        cancelText="Cancel"
        confirmColor="danger"
      ...
      <style>{`
        ...
        .tm-page-title-group { display: flex; align-items: center; gap: 12px; }
        .tm-page-icon {
          width: 42px; height: 42px; border-radius: 10px; background: #e6f1fb;
          display: flex; align-items: center; justify-content: center;
          color: #185fa5; flex-shrink: 0;
        }
        .tm-page-title { font-size: 17px; font-weight: 600; color: #0c447c; margin: 0; }
        .tm-page-sub   { font-size: 12px; color: #6b7280; margin: 0; }
        .tm-add-btn {
        ...
        .tm-spinner {
          width: 36px; height: 36px; border: 3px solid #e6f1fb;
        ...
        .tm-custom-modal .modal-content {
          border: 0.5px solid #d0dce9 !important; border-radius: 12px !important; overflow: visible !important;
        }
        .tm-modal-header {
          background: #185fa5 !important; border-bottom: none !important; padding: 16px 20px !important;
        }
        .tm-modal-title  { font-size: 15px !important; font-weight: 700 !important; color: #fff !important; }
        .tm-custom-modal .btn-close { filter: brightness(0) invert(1); opacity: 0.8; }
        .tm-modal-body   { background: #f7fafd !important; padding: 20px !important; overflow: visible !important; }
        ...
        .tm-ex-table-wrap {
          border: 0.5px solid #d0dce9; border-radius: 10px; overflow: hidden; overflow-x: auto;
        }
        .tm-ex-table { margin-bottom: 0 !important; font-size: 12px !important; }
        .tm-ex-th {
          background: #185fa5 !important; color: #fff !important;
          font-size: 11px !important; font-weight: 600 !important;
          padding: 9px 12px !important; white-space: nowrap; border: none !important;
        }
        ...
        .tm-video-link { color: #185fa5; font-weight: 500; text-decoration: none; font-size: 12px; }
