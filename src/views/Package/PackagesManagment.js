...
export default function PackagesManagement() {
  ...
  // ── Safe getter helpers ──────────────────────────────────────────────────
  /** Returns a clean array, stripping any null / undefined entries */
  ...
  const fetchData = async () => {
    try {
      ...
      // Strip any null entries the API might return
  ...
  const filteredList = list.filter((item) => {
    if (!item) return false                          // ← guard null list items
  ...
  return (
    <>
                  ...
                  <Select
                    ...
                    onChange={(val) =>
                      setForm({
                        ...form,
                        programs:   val || [],
                        programIds: val ? val.map((v) => v.value) : [],
                      })
                    }
                  />
        ...
        <CModalBody className="pm-modal-body pm-view-body">
          ...
          {!viewLoading && viewError && (
            <div className="pm-view-state pm-view-error">
              ...
              <button
                ...
                onClick={() => { setViewModal(false); setViewError(null) }}
              >
                Close
              </button>
            </div>
          ...
          {!viewLoading && !viewError && !selectedPackage && (
            <div className="pm-view-state">
              ...
              <button className="pm-btn-secondary" onClick={() => setViewModal(false)}>
                Close
              </button>
            </div>
          )}
            ...
            <div className="d-flex flex-column gap-3">
                    ...
                    <div className="pm-info-grid">
                      <div className="pm-info-row">
                        ...
                        <span className="pm-info-val">
                          {selectedPackage.noOfPrograms ??
                            safeArray(selectedPackage.programs).length}
                        </span>
                      </div>
                      <div className="pm-info-row">
              ...
              {safeArray(selectedPackage.programs).length === 0 ? (
                ...
                safeArray(selectedPackage.programs).map((program, pIndex) => {
                  // ── guard: skip null program entries ──
                        ...
                        <div className="pm-acc-body">
                          ...
                          {safeArray(program.therophyData).length === 0 ? (
                            ...
                            safeArray(program.therophyData).map((therapy, tIndex) => {
                              // ── guard: skip null therapy entries ──
                                ...
                                <div className="pm-therapy-item" key={tIndex}>
                                  <details>
                                    <summary className="pm-therapy-head">
                                      <span>
                                        {tIndex + 1}. {therapy.therapyName || "Unnamed Therapy"}
                                      </span>
                                      <span className="pm-acc-arrow">▾</span>
                                    </summary>
                                          ...
                                          <CTable bordered responsive size="sm" className="pm-ex-table">
                                            ...
                                            <CTableBody>
                                              {safeArray(therapy.exercises).map((ex, i) => {
                                                // ── guard: skip null exercise entries ──
                                                if (!ex) return null
                                                return (
                                                  <CTableRow key={i}>
      ...
      <style>{`
        ...
        .pm-no-data {
          text-align: center; color: #9ca3af; font-size: 13px; padding: 16px 0;
        }
      `}</style>
    </>
  )
